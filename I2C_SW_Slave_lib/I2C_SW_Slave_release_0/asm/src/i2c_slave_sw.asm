 ...
 */
// IO Pins
.equ	SW_SCL_OUT	= PORTC0		; (0)same pins as TWI..for ATtiny2313, use PORTD4
.equ	SW_SDA_OUT	= PORTC1		; (1)same pins as TWI..for ATtiny2313, use PORTD5
.equ	SW_SCL_IN	= PINC0			; (0)same pins as TWI..for ATtiny2313, use PIND4
.equ	SW_SDA_IN	= PINC1			; (1)same pins as TWI..for ATtiny2313, use PIND5
// IO Ports
.equ	SW_SCL_PORT	= PORTC			; (0x08)PORT reg used..for ATtiny2313, use PORTD
.equ	SW_SDA_PORT	= PORTC			; (0x08)PORT reg used..for ATtiny2313, use PORTD
// IO Read PORT
.equ	SW_SCL_PIN	= PINC			; (0x06)PIN input..for ATtiny2313, use PIND
.equ	SW_SDA_PIN	= PINC			; (0x06)PIN input..for ATtiny2313, use PIND
// IO DDRs
.equ	SW_SCL_DDR	= DDRC			; (0x07)DDR reg used..for ATtiny2313, use DDRD
.equ	SW_SDA_DDR	= DDRC			; (0x07)DDR reg used..for ATtiny2313, use DDRD
// and their interrupt control bits.
.equ	PCINT_SCL	= PCINT16		; (0)interrtup enable bit (PORTC0)..for ATtiny2313, use PCINT28
.equ	PCINT_SDA	= PCINT17		; (1)interrtup enable bit (PORTC1)..for ATtiny2313, use PCINT29
.equ	PCMSK_I2C	= PCMSK2		; (0x6d) interrupt bank (PCINT17, PCINT16)..for ATtiny2313, use PCMSK3
.equ	PCIE_I2C	= PCIE2			; (2)bank INTR enable bit..for ATtiny2313, use PCIE3

// Masks
.equ	SW_SCL_EN	= (1<<PCINT16)	; OR this
.equ	SW_SCL_DIS	= ~SW_SCL_EN	; AND this
.equ	SW_SDA_EN	= (1<<PCINT17)
...
; set state to SW_I2C_SCL_H(1)
	...
	mov		r17, r_i2c_state
;
	call	i2c_mode_service		; gets passed r17
	rjmp	pi_exit
;
; case SW_I2C_SCL_H:
pi_skip01:
